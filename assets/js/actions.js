$(document).ready(function(){
	...
	function cat(){
		$.ajax({
			url	:	base_url +"home_activity/category/",
			method:	"POST",
			data	:	{category:1},
    ...
    function cathome(){
		$.ajax({
			url	:	base_url +"home_activity/categoryhome/",
			method:	"POST",
			data	:	{categoryhome:1},
			success	:	function(data){
	...
	function brand(){
		$.ajax({
			url	:	base_url +"home_activity/brand/",
			method:	"POST",
			data	:	{brand:1},
			success	:	function(data){
				$("#get_brand").html(data);
	...
	//product() is a funtion fetching product record from database whenever page is load
		...
		$.ajax({
			url	:	base_url +"home_activity/getproducts",
			method:	"POST",
			data	:	{getProduct:1},
			success	:	function(data){
		...
		var cid = $(this).attr('cid');
			...
			success	:	function(data){
				...
				if(window.location.pathname=='/myshop/'){
					window.location.assign(base_url +"store/"+cid+"#");
				}
		...
		$.ajax({
			...
			success	:function(data){
				if(data == "login_success"){
					...
					window.location.href = base_url +"home/cartview";
				}else{
					$("#e_msg").html(data);
					$(".overlay").hide();
				}
	...
	*/
    ...
    $("body").delegate(".remove","click",function(event){
        ...
        $.ajax({
            url	:	base_url +"carts/removeItemFromCart",
            method	:	"POST",
	...
	*/
	...
	function checkOutDetails(){
	 $('.overlay').show();
		$.ajax({
			url : base_url +"carts/cart_checkout",
			method : "POST",
	...
	*/
	function net_total(){
		...
		$('.net_total').html("Total : <i class='fa fa-inr'></i> " +net_total);
	}
	...
	function page(){
		$.ajax({
			url	:	base_url +"home_activity/pagination",
			method	:	"POST",
			data	:	{page:1},
			success	:	function(data){
				$("#pageno").html(data);
